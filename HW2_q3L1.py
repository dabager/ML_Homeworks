...
while (clusterComparer() and lastErrorChecker() and iteration < maxNumberOfIterations):
    ...
    for i in range(clusterCount):
        table['dist' + str(i)] = 0
        for j in range(1, 65):
            table['dist' + str(i)] = table['dist' + str(i)] + abs(table['f' + str(j)] - clusterCenters[i][j - 1])
    ...
    clusterTable['cluster'] = clusterTable['cluster'].str.replace('dist','')

    for i in range(clusterCount):
        clusterTable = clusterTable.drop('dist' + str(i), axis = 1)
        table = table.drop('dist' + str(i), axis = 1)
